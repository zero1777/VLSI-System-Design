		...
		for (int p=0; p<P; p++) {
			for (int q=0; q<Q; q++) {
				for (int c=0; c<C; c++) {
					for (int r=0; r<R; r++) {
						for (int s=0; s<S; s++) {
							*(output_data + num*P*Q + p*Q + q) += *(input_data + c*H*W + (p+r)*W + (q+s)) * *(weight_data + num*C*R*S + c*R*S + r*S + s);
						}
					}
				}
			}
...
void soft_fc(int *input_shape, int *weight_shape, int *output_shape, volatile int8_t* input_data, volatile int8_t* weight_data, volatile int32_t* output_data)
{
	...
	for (int r=0; r<R; r++) {
		for (int h=0; h<H; h++) {
			*(output_data + r) += *(input_data + h) * *(weight_data + r * S + h);
		}
	} 
}
...
void unfold(int *input_shape, int *weight_shape, int *output_shape, volatile int8_t* input_data, volatile int8_t* output_data)
{
	...
	int op_N = output_shape[0];
	int op_C = output_shape[1];
	int op_H = output_shape[2];
	int op_W = output_shape[3];
				...
				for (int j=0; j<W-S+1; j++) {
					for (int k1=0; k1<S; k1++) {
						for (int k2=0; k2<S; k2++) {
							*(output_data + b*op_C*op_H*op_W + c*op_H*op_W + row_num*op_W + k1*S + k2) = *(input_data + b*C*H*W + c*H*W + (i+k1)*W + (j+k2));
						}
					}
					row_num++;
				}
