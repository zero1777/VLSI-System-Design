...
#define CODESEQ_OFFSET				0x00050000
#define TREE_OFFSET					0x00060000
#define GAPARR_OFFSET				0x00070000
#define PREFIX_OFFSET				0x00080000
#define IDX_OFFSET					0x00090000
#define OUTPUT_OFFSET				0x000a0000
#define HEAP_OFFSET					0x000b0000
...
#define DNN_MMAP_BASE 				0x40000000
#define DNN_MMAP_RANG 				0x0000ffff
#define DNN_MMAP_DECODE_FINISH		0x0
#define DNN_MMAP_GAPARR_OFFSET		0x4
#define DNN_MMAP_IDX_OFFSET			0x8
#define DNN_MMAP_CODESEQ_OFFSET		0xc
#define DNN_MMAP_HEAP_OFFSET		0x10
#define DNN_MMAP_WRITE_LEN			0x14
#define DNN_MMAP_DECODE_START		0x18
#define DNN_MMAP_ENCODE_FINISH		0x1c
#define DNN_MMAP_ENCODE_START		0x20
#define DNN_MMAP_INPUT_OFFSET		0x24
#define DNN_MMAP_LENGTH_OFFSET		0x28
#define DNN_MMAP_SYMBOL_OFFSET		0x2c
#define DNN_MMAP_ENCODE_OFFSET		0x30
#define DNN_MMAP_TREE_OFFSET		0x34
#define DNN_MMAP_PREFIX_OFFSET		0x38
#define DNN_MMAP_OUTPUT_OFFSET		0x3c
...
void SOFT() {
	...
	volatile int8_t* codeseq_data = CODESEQ_OFFSET;
	volatile signed char * tree_data = TREE_OFFSET;
	int codeseq_length = 72;

	int start, stop;

	// encode
	start = tick();
	huffman_encode_soft(input_data, symbol_data, length_data, encode_data, codeseq_data);
	stop = tick();
	print_str("soft encode time usage: ");
	print_dec(stop-start);
	print_str("\n");

	// decode
	start = tick();
	huffman_decode_soft(codeseq_data, tree_data, codeseq_length);
	stop = tick();
	print_str("soft decode time usage: ");
	print_dec(stop-start);
	print_str("\n");
}

void huffman_encode_soft(volatile signed char * input_data, volatile signed char * symbol_data, volatile int8_t* length_data, volatile int8_t* encode_data, volatile int8_t* codeseq_data)
{
	// print_str("helllllo   ");
	// Your data location
	// volatile signed char * input_data = INPUT_OFFSET;
...
void huffman_decode_soft(volatile int8_t* codeseq_data, volatile signed char * tree_data, int codeseq_length)
{
	// print_str("helllllo   ");
	// Your data location
	// volatile signed char * input_data = INPUT_OFFSET;
	...
	// 	// print_dec(*(symbol_data+i));
	// 	print_str("\n");
	// 	print_chr(*(tree_data + i));
	// }

	// print symbol data 
	// for(int i=0; i<5; i++){
	// 	print_chr(*(codeseq_data+i)%2);
	// 	// print_chr(*(codeseq_data+i)/16);
	// 	print_str("\n");
	// 	// print_chr(*(symbol_data + i));
	// }
	...
	// int codeseq_length = 33;
	print_str("\n");
	int idx = 0;
	unsigned int code, cd;
	int scan = 0;
	while (idx < codeseq_length) {
		if (idx % 8 == 0) {
			code = *(codeseq_data + (idx/8));
		}
		cd = code % 2;
		code /= 2;
		if (cd == 0) {
...
void HARD() {
	...
	int start, stop;
	int output_length = 136;
	int codeseq_length;
	int LEN = 43;

	// Your data location
	volatile int8_t* _output_data = OUTPUT_OFFSET;
	volatile int8_t* _codeseq_data = CODESEQ_OFFSET;

	// *(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_INPUT_OFFSET) = 
	// *(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_LENGTH_OFFSET) = 
	// *(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_SYMBOL_OFFSET) = 
	// *(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_ENCODE_OFFSET) = 
	...
	// print_str("\nThe result of hard_encode is:\n");
	// for(int i=0; i< codeseq_length; i++){
	// 	print_dec(i);
	// 	print_str(": ");
	// 	print_sign(*(_codeseq_data + i));
	// 	print_str("\n");
	// }
	// print_str("\nThe time usage: ");
	// print_dec(stop - start);
	...
	start = tick();
	huffman_decode_hard(LEN);
	stop = tick();

	// Print your conv output
	// output_length = ;

	print_str("\nThe result of hard_decode is:\n");
	for(int i=0; i< output_length; i++){
		print_dec(i);
		print_str(": ");
		print_chr(*(_output_data + i));
		print_str("\n");
	}
	print_str("\nThe time usage: ");
	print_dec(stop - start);
...
void huffman_encode_hard() {
	// Start the convolution layer
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_ENCODE_START) = 1;

	// If not using blocking output, SW needs to check the status
	// to make sure that HW output is ready.  Please refer to dnn_mmap.v
	while(*(volatile uint32_t*)(DNN_MMAP_BASE + DNN_MMAP_ENCODE_FINISH) == 0){
	}

	return;
}

void huffman_decode_hard(int LEN) {
	// Start the convolution layer
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_CODESEQ_OFFSET) = CODESEQ_OFFSET;
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_HEAP_OFFSET) = HEAP_OFFSET;
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_GAPARR_OFFSET) = GAPARR_OFFSET;
	// *(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_PREFIX_OFFSET) = 
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_IDX_OFFSET) = IDX_OFFSET;
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_OUTPUT_OFFSET) = OUTPUT_OFFSET;
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_WRITE_LEN) = LEN;
	*(volatile uint32_t *)(DNN_MMAP_BASE + DNN_MMAP_DECODE_START) = 1;

	// If not using blocking output, SW needs to check the status
	// to make sure that HW output is ready.  Please refer to dnn_mmap.v
	while(*(volatile uint32_t*)(DNN_MMAP_BASE + DNN_MMAP_DECODE_FINISH) == 0){
	}
